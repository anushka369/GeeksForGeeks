        ...
        void correctBST(Node* root) 
        {
            ...
            function<void(Node*)> inorder = [&](Node* node) 
            {
                ...
                if (prev && prev -> data > node -> data) 
                {
                    if (!first) 
                    {
                        first = prev; // First node is the previous node
                    }
                    second = node; // Second node is the current node
                }
...
// Link to the problem: https://www.geeksforgeeks.org/problems/fixed-two-nodes-of-a-bst/1/
